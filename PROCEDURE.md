...
- `js/sections.jsx` and `js/app.jsx`: the components. **Edit the `.jsx`, never
  the `.js`.** Compile with `npm run build` (runs `tools/build.js`), which
  regenerates `js/sections.js` and `js/app.js`.
- `portfolio.css` / `styles.css` / `tokens/`: the styling.

### Adding a completed project to the front page
...
3. **Render:** the site pulls React from a CDN, so to render it offline, point
   ...
   load it with headless Chromium. Confirm the index cards, the stories in
   order, the new charts and table, and every update timeline render with no
   console errors.
